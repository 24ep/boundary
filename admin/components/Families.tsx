...
export function Families() {
  ...
  const [formData, setFormData] = useState({
    name: '',
    description: '',
    type: 'family' as 'family' | 'friends' | 'sharehouse'
  })
  ...
  const handleSave = async () => {
    try {
      if (editingFamily) {
        await adminService.updateFamily(editingFamily.id, formData)
      } else {
        await adminService.createFamily(formData)
      }
  ...
  const handleDelete = async (family: Family) => {
    if (confirm(`Are you sure you want to delete "${family.name}"?`)) {
      try {
        await adminService.deleteFamily(family.id)
        await loadFamilies()
      } catch (err) {
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-lg text-gray-600 leading-relaxed">Create, edit, and manage family groups and their members</p>
          </div>
          <button 
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
          ...
          <p className="text-gray-600 mb-6">Try adjusting your search or create a new family.</p>
          <button 
            onClick={handleCreate}
