  ...
  const loadAnalytics = async () => {
    try {
      ...
      const analyticsData: AnalyticsData = {
        ...
        featuredContent: content.filter(c => c.is_featured).length,
        totalViews: content.reduce((sum, c) => sum + (c.view_count || 0), 0),
        totalLikes: content.reduce((sum, c) => sum + (c.like_count || 0), 0),
        totalComments: content.reduce((sum, c) => sum + (c.comment_count || 0), 0),
        contentByType: Object.entries(
          content.reduce((acc, c) => {
            const type = c.content_types?.name || 'unknown'
