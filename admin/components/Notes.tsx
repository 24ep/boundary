  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <Card variant="frosted">
        <CardBody>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-2">Family Notes</h2>
      ...
      <Card variant="frosted">
        <CardBody>
          <div className="flex items-center gap-4">
            <label className="text-sm font-medium text-gray-700">Select Family:</label>
            <Select
              value={selectedFamily}
              onChange={(e) => setSelectedFamily(e.target.value)}
              options={families.map(family => ({
                value: family.id,
                label: `${family.name} (${family.memberCount} members)`
              }))}
              className="w-auto"
            />
          </div>
      ...
      <Modal
        ...
        title={`${editingNote ? 'Edit Note' : 'Create Note'} for ${getSelectedFamilyName()}`}
      >
          ...
          <div className="flex gap-4">
            ...
            <label className="flex items-center cursor-pointer">
              <input
                ...
                className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
              />
              <span className="ml-2 text-sm text-gray-700">Share with Family</span>
            </label>
          </div>
