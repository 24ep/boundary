...
// Provider icons mapping with actual brand logos
const providerIcons: Record<string, any> = {
  google: GoogleIcon,
  'google-oauth': GoogleIcon,
  github: GitHubIcon,
  'github-oauth': GitHubIcon,
  twitter: XIcon,
  'x-oauth': XIcon,
  facebook: FacebookIcon,
  'facebook-oauth': FacebookIcon,
  microsoft: MicrosoftIcon,
  'microsoft-oauth': MicrosoftIcon,
  apple: Layers,
}
...
function LoginPageContent() {
  ...
  useEffect(() => {
    if (authService.isAuthenticated()) {
        ...
        // If redirecting to OAuth authorize, sync the session cookie first to avoid redirect loops
        const needsCookieSync = redirect.includes('/oauth/authorize')
        if (needsCookieSync) {
          const token = authService.getToken()
          if (token) {
            fetch('/api/v1/admin/auth/session-sync', {
              method: 'POST',
              headers: { 'Authorization': `Bearer ${token}` },
            }).catch(() => {}).finally(() => navigateAfterAuth(redirect))
            return
          }
        }
  ...
  return (
    <div className="h-screen w-full flex flex-col md:flex-row relative overflow-hidden text-gray-900" style={loginBgStyle}>
      ...
      <div className="w-full md:w-[40%] min-w-[320px] p-4 md:p-6 flex flex-col justify-center relative z-10 h-full">
        ...
        >
          <CardHeader className="space-y-3 pb-2 flex flex-col items-center pt-8">
            {logoUrl && (
              <div className="mb-1 p-3 rounded-xl border border-gray-200 bg-white shadow-sm">
                <img src={logoUrl} alt="Logo" className="h-10 w-10 object-contain" />
              </div>
            )}
            ...
            <CardDescription className="text-center text-gray-500 text-base">
              {formMode === 'signin' ? 'Welcome back to ' + appName : 'Join ' + appName}
            </CardDescription>
          </CardHeader>
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <Input
                    ...
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    className="bg-white/70 border-gray-300 focus:bg-white focus:border-blue-400 transition-all hover:bg-white/90"
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <Input
                    ...
                    value={password}
                    onChange={(e) => setPassword(e.target.value)}
                    className="pr-10 bg-white/70 border-gray-300 focus:bg-white focus:border-blue-400 transition-all hover:bg-white/90"
                    style={{ paddingLeft: '2.5rem' }}
                    required
                  />
                  <Button
            ...
            {ssoProviders.length > 0 && (
              <>
                <div className="flex items-center gap-3 my-4">
                  ...
                  <span className="text-xs uppercase font-medium text-gray-400 whitespace-nowrap">or continue with</span>
                  <div className="flex-1 h-px bg-gray-200" />
                </div>
                  ...
                  {ssoProviders.map((provider: any) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        className="w-full flex items-center gap-3 h-11 px-4 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:border-gray-400 transition-all text-sm font-medium text-gray-700 shadow-sm"
                      ...
                      >
                        ...
                        <span>
                          {ssoLoading === provider.providerName
                            ? 'Connecting...'
                            : `Continue with ${provider.displayName || provider.providerName}`}
                        </span>
                      </button>
                    )
                  })}
