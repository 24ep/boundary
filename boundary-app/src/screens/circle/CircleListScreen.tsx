...
const CircleListScreen: React.FC = () => {
  const navigation = useNavigation<any>();
  const [families, setFamilies] = useState<CircleItem[]>([]);
  ...
  const loadFamilies = async () => {
    try {
      ...
      const transformedFamilies: CircleItem[] = apiFamilies.map((circle: any) => ({
        id: circle.id,
        name: circle.name,
        description: circle.description || `${circle.membersCount} members • ${circle.type || 'Private'}`,
        avatar: undefined, // Could be enhanced with circle avatar
  ...
  const renderItem = ({ item }: { item: CircleItem }) => (
    <TouchableOpacity
      style={styles.card}
