...
interface PersonalGalleryTabProps {
  onPhotoPress?: (photo: Photo) => void;
  onAlbumPress?: (album: Album) => void;
}

export const PersonalGalleryTab: React.FC<PersonalGalleryTabProps> = ({
  onPhotoPress,
  onAlbumPress,
}) => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [photosResult, albumsData, statsData] = await Promise.all([
        galleryService.getPersonalPhotos(filters),
        galleryService.getPersonalAlbums(),
      ...
      console.error('[PersonalGalleryTab] Error loading data:', error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleToggleFavorite = async (photoId: string) => {
    try {
      ...
      console.error('[PersonalGalleryTab] Error toggling favorite:', error);
    }
  };
  ...
  const handleDeletePhoto = async (photoId: string) => {
    try {
      ...
      console.error('[PersonalGalleryTab] Error deleting photo:', error);
    }
  };
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyState}>
      ...
      <Text style={styles.emptyTitle}>
        {viewMode === 'photos' ? 'No Photos Yet' : 'No Albums Yet'}
      </Text>
...
export default PersonalGalleryTab;
