# Privacy Policy

**Last Updated**: January 1, 2024

## 1. Introduction

Welcome to Bondarys ("we," "our," or "us"). We are committed to protecting your privacy and ensuring the security of your family's information. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our family safety and communication platform.

### Contact Information
- **Company**: Bondarys Inc.
- **Email**: privacy@bondarys.com
- **Address**: [Company Address]
- **Website**: https://bondarys.com

## 2. Information We Collect

### 2.1 Information You Provide
- **Account Information**: Name, email address, phone number, family relationships
- **Family Information**: Family member names, ages, relationships, emergency contacts
- **Location Data**: Real-time location, location history, geofence settings
- **Communication Data**: Messages, voice calls, video calls, photos, files
- **Safety Information**: Emergency contacts, safety zones, panic alerts
- **Payment Information**: Credit card details, billing address (processed securely)

### 2.2 Information We Collect Automatically
- **Device Information**: Device type, operating system, unique device identifiers
- **Usage Data**: App usage patterns, feature interactions, session duration
- **Location Data**: GPS coordinates, location accuracy, movement patterns
- **Technical Data**: IP address, browser type, app version, crash reports

### 2.3 Information from Third Parties
- **Social Media**: When you connect social media accounts
- **Emergency Services**: When emergency alerts are triggered
- **Analytics Providers**: Anonymous usage statistics

## 3. How We Use Your Information

### 3.1 Core Services
- **Family Communication**: Enable family chat, calls, and messaging
- **Location Sharing**: Provide real-time location tracking and history
- **Safety Features**: Implement geofencing, emergency alerts, panic button
- **Family Organization**: Manage calendar, photos, files, and events

### 3.2 Service Improvement
- **App Functionality**: Improve features and user experience
- **Performance**: Monitor app performance and fix issues
- **Analytics**: Analyze usage patterns to enhance services
- **Security**: Detect and prevent fraud and abuse

### 3.3 Communication
- **Service Updates**: Important app updates and maintenance
- **Safety Alerts**: Emergency notifications and safety information
- **Marketing**: Promotional content (with your consent)
- **Support**: Customer service and technical support

## 4. Information Sharing and Disclosure

### 4.1 Family Members
- **Location Data**: Shared with family members you authorize
- **Communication**: Messages and calls within your family group
- **Photos and Files**: Shared content within your family
- **Safety Information**: Emergency contacts and safety settings

### 4.2 Service Providers
- **Cloud Storage**: Secure data storage and backup
- **Payment Processing**: Secure payment transactions
- **Analytics**: Anonymous usage statistics
- **Support**: Customer service and technical support

### 4.3 Legal Requirements
- **Law Enforcement**: When required by law or court order
- **Emergency Services**: During emergency situations
- **Legal Proceedings**: In response to legal requests
- **Safety**: To protect user safety and prevent harm

### 4.4 Business Transfers
- **Merger or Acquisition**: In connection with business transfers
- **Asset Sales**: When selling company assets
- **Bankruptcy**: In bankruptcy proceedings

## 5. Data Security

### 5.1 Security Measures
- **Encryption**: End-to-end encryption for all communications
- **Access Controls**: Strict access controls and authentication
- **Data Centers**: Secure, certified data centers
- **Monitoring**: 24/7 security monitoring and threat detection

### 5.2 Data Retention
- **Active Accounts**: Data retained while account is active
- **Inactive Accounts**: Data deleted after 12 months of inactivity
- **Location History**: Retained for 90 days (configurable)
- **Emergency Data**: Retained for legal and safety purposes

### 5.3 Data Breach Response
- **Detection**: Immediate detection and assessment
- **Notification**: Prompt notification to affected users
- **Investigation**: Thorough investigation and remediation
- **Prevention**: Implementation of additional security measures

## 6. Your Rights and Choices

### 6.1 Access and Control
- **View Data**: Access your personal information
- **Update Data**: Correct or update your information
- **Delete Data**: Request deletion of your data
- **Export Data**: Download your data in portable format

### 6.2 Privacy Settings
- **Location Sharing**: Control who can see your location
- **Communication**: Manage who can contact you
- **Data Sharing**: Control data sharing with third parties
- **Notifications**: Manage notification preferences

### 6.3 Opt-Out Options
- **Marketing**: Opt out of marketing communications
- **Analytics**: Opt out of analytics tracking
- **Location Tracking**: Disable location services
- **Data Sharing**: Limit data sharing with third parties

## 7. Children's Privacy (COPPA)

### 7.1 Children Under 13
- **Parental Consent**: We require parental consent for children under 13
- **Limited Data**: We collect minimal data from children
- **Parental Control**: Parents have full control over children's accounts
- **Educational Content**: We provide educational content about online safety

### 7.2 Parental Rights
- **Review Data**: Parents can review children's data
- **Delete Data**: Parents can delete children's data
- **Modify Settings**: Parents can modify privacy settings
- **Revoke Consent**: Parents can revoke consent at any time

## 8. International Data Transfers

### 8.1 Data Transfers
- **Global Service**: We provide services globally
- **Data Centers**: Data may be stored in different countries
- **Legal Compliance**: We comply with local privacy laws
- **Safeguards**: We implement appropriate safeguards for transfers

### 8.2 Regional Compliance
- **GDPR**: European Union data protection regulations
- **CCPA**: California Consumer Privacy Act
- **COPPA**: Children's Online Privacy Protection Act
- **Local Laws**: Compliance with local privacy laws

## 9. Cookies and Tracking Technologies

### 9.1 Types of Cookies
- **Essential Cookies**: Required for app functionality
- **Analytics Cookies**: Help us understand app usage
- **Marketing Cookies**: Used for advertising (with consent)
- **Preference Cookies**: Remember your preferences

### 9.2 Cookie Management
- **Browser Settings**: Manage cookies through browser settings
- **App Settings**: Control tracking through app settings
- **Opt-Out Tools**: Use opt-out tools for advertising
- **Third-Party Cookies**: Control third-party cookie access

## 10. Third-Party Services

### 10.1 Integrated Services
- **Maps**: Google Maps for location services
- **Payments**: Stripe for payment processing
- **Analytics**: Firebase for app analytics
- **Storage**: AWS for data storage

### 10.2 Third-Party Policies
- **Independent Services**: Third parties have their own privacy policies
- **Data Sharing**: We share data only as necessary for services
- **Security**: We require third parties to maintain security standards
- **Compliance**: Third parties must comply with privacy laws

## 11. Changes to This Privacy Policy

### 11.1 Policy Updates
- **Regular Review**: We review this policy regularly
- **Material Changes**: We notify users of material changes
- **Effective Date**: Changes become effective on the date specified
- **Continued Use**: Continued use constitutes acceptance of changes

### 11.2 Notification Methods
- **In-App Notification**: Notifications within the app
- **Email Notification**: Email notifications to registered users
- **Website Updates**: Updates posted on our website
- **Social Media**: Announcements on social media platforms

## 12. Contact Us

### 12.1 Privacy Questions
If you have questions about this Privacy Policy or our privacy practices, please contact us:

- **Email**: privacy@bondarys.com
- **Phone**: [Phone Number]
- **Address**: [Company Address]
- **Online Form**: https://bondarys.com/contact

### 12.2 Data Protection Officer
For European users, you can contact our Data Protection Officer:

- **Email**: dpo@bondarys.com
- **Address**: [DPO Address]

### 12.3 Complaints
If you believe we have not addressed your privacy concerns, you may:

- **Contact Us**: Reach out to our privacy team
- **Regulatory Authority**: Contact your local data protection authority
- **Legal Action**: Pursue legal remedies as available

## 13. Legal Basis for Processing (GDPR)

### 13.1 Legal Grounds
- **Consent**: Processing based on your consent
- **Contract**: Processing necessary for service provision
- **Legitimate Interest**: Processing for our legitimate interests
- **Legal Obligation**: Processing required by law

### 13.2 Data Subject Rights
- **Right to Access**: Access your personal data
- **Right to Rectification**: Correct inaccurate data
- **Right to Erasure**: Request deletion of your data
- **Right to Portability**: Receive your data in portable format
- **Right to Object**: Object to certain processing
- **Right to Restriction**: Restrict processing of your data

## 14. California Privacy Rights (CCPA)

### 14.1 California Residents
California residents have additional rights under the CCPA:

- **Right to Know**: Know what personal information we collect
- **Right to Delete**: Request deletion of personal information
- **Right to Opt-Out**: Opt out of sale of personal information
- **Right to Non-Discrimination**: Not be discriminated against for exercising rights

### 14.2 CCPA Contact
For CCPA requests, contact us at:

- **Email**: ccpa@bondarys.com
- **Phone**: [CCPA Phone Number]
- **Online Form**: https://bondarys.com/ccpa

## 15. Additional Information

### 15.1 Data Processing Details
- **Data Controller**: Bondarys Inc.
- **Data Processor**: Various service providers
- **Data Categories**: Personal, location, communication, safety data
- **Processing Purposes**: Service provision, safety, communication, analytics

### 15.2 Security Certifications
- **SOC 2**: Service Organization Control 2 certification
- **ISO 27001**: Information security management certification
- **GDPR Compliance**: European data protection compliance
- **Regular Audits**: Annual security and privacy audits

---

**This Privacy Policy is effective as of January 1, 2024. We encourage you to review this policy regularly and contact us with any questions.** 